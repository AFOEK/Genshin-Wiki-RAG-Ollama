...
def process_document(conn, embed_fn, config, source, url, title, raw_text, tier="primary", weight=1.0):
    ...
    if row is None:
        cur.execute("SELECT doc_id, raw_hash FROM docs WHERE url=? AND raw_hash=? ORDER BY doc_id DESC LIMIT 1", (source, raw_hash))
        moved = cur.fetchone()
        if moved:
        ...
        if row:
            ...
            if old_raw_hash == raw_hash:
                ...
                if active_chunks > 0:
                    ...
                    conn.commit()
                    return
            log.warning("REBUILD %s (content changed)", url)
